...
/**
 * 회사(업무) 프로젝트 — 시간 역순 정렬
 * 개인 사이드 프로젝트는 data/sideProjects.ts 참조
 */
export const projects: Project[] = [
  {
    ...
    period: "2024.11 ~ 현재",
    description:
      "물류 포워딩 B2B SaaS 플랫폼(ILIC) 신규 구축에 풀스택으로 참여 및 리드. 포워더, 화주, 파트너 3개 사용자 타입 지원 멀티테넌트 SaaS. 22개 업무 도메인, 3개 리포지토리(백엔드 + 백오피스 + 프론트오피스) 단독 운영. 팀에서 Claude Code를 도입해 함께 사용하는 워크플로우를 만들어 쓰고 있음.",
    longDescription:
      "포워더, 화주, 파트너 3개 사용자 타입 지원 멀티테넌트 SaaS. 22개 업무 도메인(Booking, Shipment, Settlement, Pricing 등), 3개 리포지토리(백엔드 + 백오피스 + 프론트오피스) 단독 운영. 관세청, 환율, 은행, 다국어 SMS 등 외부 API 연동. 현재 MVP 완성 후 내부 테스트 단계. 기술 스택 — Backend: Java 17, Spring Boot 3.3.5, JPA, QueryDSL 5, Spring Security(JWT), OpenFeign, Quartz + ShedLock, MySQL 8, Redis, Apache POI, Jasypt, Lucy-XSS / Frontend: Vue 3.5, TypeScript, Vite, Pinia, Vuetify, Vue-i18n, Vee-Validate, TipTap, ApexCharts / DevOps: Docker, Jenkins, CI/CD, Git / Realtime: SSE(인앱 실시간 알림). 아키텍처 설계 — 레이어 의존 방향 강제(api → domain → global), Multi-Datasource(biz + common 2개 DB), QueryDSL Custom Repository 3단 구조, 외부 연동 Feign Client 추상화. Claude Code 활용 — CLAUDE.md 컨텍스트 문서를 작성해 3개 리포지토리에 적용, 다국어 i18n sync/translate/validate 스크립트 7종을 만들어서 사용, DB 스키마 분석·엑셀 추출·권한 동기화 업무용 스크립트 작성. 연구전담부서 지정 기여 — 기술 기획 문서, 개발 일지, 연구 성과 문서 작성 주도, 회사의 연구전담부서 인증·지정 과정에 필요한 문서 체계 정비. 로드맵 — PWA(브라우저 Push) → Capacitor 모바일 앱(Vue 3 → Android, iOS) 확장.",
    tags: [
      "Spring Boot 3.3.5",
      "Vue 3.5",
      "TypeScript",
